...
class URLRepository:
    ...
    def delete_expired_urls(self) -> int:
        ...
        expired_urls = self.db.query(URL).filter(
            URL.expiration_time.isnot(None),
            URL.expiration_time < datetime.utcnow()
        ).all()
    ...
    def create_with_error_handling(self, original_url: str, short_code: str, expiration_time: Optional[datetime] = None) -> Optional[URL]:
        """
